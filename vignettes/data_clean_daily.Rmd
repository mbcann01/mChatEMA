...
# Calculate time to complete the assessment

```{r assess_time}
daily <- daily %>%
  mutate(assess_time = (end_time - start_time) / 60)
```
...
daily <- daily %>%
  mutate(
    ...
    long = format(round(long, 13), nsmall = 13),
    lat  = format(round(lat, 13), nsmall = 13)
  )

# Data check
# View(select(test, gps, long, lat))
```
...
# Data check
about_data(daily)
# 2016-07-22: 7,378 observations and 134 variables in the data

test_that("The number of variables hasn't changed", {
  expect_equal(ncol(daily), 134)
})
```
...
# Data check:
about_data(daily)
# 2016-07-22: 7,378 observations and 164 variables in the data

test_that("The number of variables hasn't changed", {
  expect_equal(ncol(daily), 164)
})
```
...
about_data(daily)
# 2016-07-22: 7,378 observations and 186 variables in the data

test_that("The number of variables hasn't changed", {
  expect_equal(ncol(daily), 186)
})
```

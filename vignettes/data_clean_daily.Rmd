...
daily <- mutate(daily, 
  ...
  race_eth = case_when(
    ...
    daily$race == "Black" & daily$hispanic == "No" ~ "Black, non-Hispanic",
...
# Data check
about_data(daily)
# 2016-07-22: 7,378 observations and 135 variables in the data

test_that("The number of variables hasn't changed", {
  expect_equal(ncol(daily), 135)
})
```
...
about_data(daily)
# 2016-07-22: 7,378 observations and 187 variables in the data

test_that("The number of variables hasn't changed", {
  expect_equal(ncol(daily), 187)
})
```

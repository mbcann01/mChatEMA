...
```{r plot_emotions}
x <- names(select(daily, happy, frustrated, sad, worried, restless, excited, calm, lonely, bored, sluggish))
for (var in x) {
...
```{r plot_location}
x <- names(select(daily, bus_loc, church_loc, home_loc, frifam_loc, store_loc, outdoors_loc, other_loc, restaurant_loc, school_loc, work_loc))
...
```{r plot_act}
x <- names(select(daily, other_act, sit_act, sleep_act, stand_act, walk_act, talk_act, pre_who))
...
```{r plot_act_yest}
x <- names(select(daily, bike_yest, run_yest, cleaning_yest, none_yest, min_walk, min_run, min_act, min_sit))
